...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	foreach ($all_user as $value1) {
		foreach($_POST as $key => $value2)
			{
				$found=0;
				if ($value1 == $value2)
				{// echo " This value: ".$value1." is in! <br />";
					$found=1;
		...
		if ($found==0)
			{//echo "This value: ".$value1." is NOT FOUND! <br />";
...
while ($row2 = $result2->fetch_assoc()){$user_data[]=$row2['follow_by'];}

function checkUser($user,$user_data ){if (in_array("$user", $user_data)) {echo "checked";}
